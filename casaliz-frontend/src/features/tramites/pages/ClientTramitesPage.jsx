  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#f8f5ef]">
        <div className="container-custom flex min-h-[60vh] items-center justify-center text-[#233274]">
    ...
    <main className="min-h-screen bg-[#f8f5ef]">
      <section className="border-b border-[#e7e0d5] bg-white">
        <div className="container-custom py-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-black uppercase text-[#e15f0b]">Portal del cliente</p>
              <h1 className="mt-1 text-3xl font-black text-[#233274]">Mis tramites</h1>
              <p className="mt-2 max-w-2xl text-[#69656c]">
                Revisa tus tramites activos, historial con Casaliz y siguientes servicios recomendados.
              </p>
            </div>
            <Link
              to="/consulta-tramite"
              className="inline-flex min-h-[44px] items-center justify-center rounded-lg border border-[#233274] px-4 py-2 text-sm font-black text-[#233274] transition hover:bg-[#233274] hover:text-white"
            >
      ...
      <section className="container-custom py-8">
        {error ? (
          ...
          <div className="rounded-lg border border-[#e7e0d5] bg-white p-8 text-center">
            <p className="text-xl font-black text-[#233274]">Aun no tienes tramites vinculados a tu cuenta</p>
            <p className="mt-2 text-sm text-[#69656c]">
              Si ya tienes un codigo, puedes consultar el estado sin iniciar sesion o pedir al equipo que lo vincule a tu perfil.
            </p>
            <Link
              to="/consulta-tramite"
              className="mt-5 inline-flex min-h-[44px] items-center justify-center rounded-lg bg-[#e15f0b] px-5 font-black text-white transition hover:bg-[#c84f06]"
            >
              Consultar por codigo
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-[360px_1fr]">
            <aside className="space-y-4">
              <div className="rounded-lg border border-[#e7e0d5] bg-white p-4 shadow-sm">
                <label htmlFor="client-tramite-search" className="text-xs font-black uppercase text-[#69656c]">
                  Buscar
                </label>
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    className="min-h-[44px] w-full rounded-lg border border-[#d8d1c6] pl-10 pr-3 text-sm text-[#233274] outline-none focus:border-[#e15f0b]"
                  />
                </div>
              </div>
                  ...
                  <button
                    ...
                    className={`w-full rounded-lg border p-4 text-left transition ${
                      selected?.id === tramite.id
                        ? 'border-[#e15f0b] bg-white shadow-sm'
                        : 'border-[#e7e0d5] bg-white hover:border-[#233274]'
                    }`}
                  >
                    <p className="text-xs font-black uppercase text-[#e15f0b]">{tramite.code}</p>
                    <p className="mt-1 font-black text-[#233274]">{tramite.project_name || tramite.type?.name}</p>
                    <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#ece7dd]">
                      <div className="h-full rounded-full bg-[#e15f0b]" style={{ width: `${tramite.progress || 0}%` }} />
                    </div>
                    <p className="mt-2 text-xs font-semibold text-[#69656c]">{tramite.status_label} - {tramite.progress || 0}%</p>
                  </button>
                ))}
              </div>
            </aside>
            ...
            <div className="space-y-6">
              ...
              <section className="rounded-lg border border-[#e7e0d5] bg-white p-5 shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-black text-[#233274]">Servicios recomendados</h2>
                </div>
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                  {recommendations.map((item) => (
                    <div key={item.title} className="rounded-lg border border-[#e7e0d5] bg-[#fdfaf5] p-4">
                      <p className="font-black text-[#233274]">{item.title}</p>
                      <p className="mt-1 text-sm text-[#69656c]">{item.description}</p>
                    </div>
                  ))}
                </div>
              </section>
...
const Stat = ({ icon, label, value }) => (
  <div className="rounded-lg border border-[#e7e0d5] bg-[#fdfaf5] p-4">
    {createElement(icon, { className: 'h-5 w-5 text-[#e15f0b]' })}
    <p className="mt-3 text-3xl font-black text-[#233274]">{value || 0}</p>
    <p className="text-sm font-bold text-[#69656c]">{label}</p>
  </div>
);
